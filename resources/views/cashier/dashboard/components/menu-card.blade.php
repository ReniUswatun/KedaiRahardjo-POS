<div class="min-w-[235px] bg-white rounded-2xl shadow-md border px-4 pt-4 pb-2 flex flex-col">
  ...
  <div class="flex-1">
    ...
    {{-- Judul Menu --}}
    <h3 class="text-lg font-semibold">{{ $menu->product_name }}</h3>
  ...
  <div class="flex items-center justify-center gap-4 pb-2 mx-3 mt-2">
    <!-- Tombol minus -->
    <button 
      @click="subtract({ id: {{ $menu->id }}, name: '{{ $menu->product_name }}', price: {{ $menu->selling_price }} })"
    ...
    <div class="bg-gray-100 text-black text-xl font-medium px-6 py-3 rounded-xl w-14 text-center">
      <template x-if="getMenu().find(i => i.id === {{ $menu->id }})">
        <span x-text="getMenu().find(i => i.id === {{ $menu->id }}).quantity"></span>
      </template>
      <template x-if="!getMenu().find(i => i.id === {{ $menu->id }})">
        <span>0</span>
      </template>
